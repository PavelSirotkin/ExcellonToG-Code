...
class TestPerformance:
    ...
    def test_tsp_slots_optimization_performance(self):
        ...
        # 10s порог — после серии правок 2026-05-02: тест стабильно укладывался
        # в ~3-4s в изоляции, но под нагрузкой полного suite на медленных
        # CI-раннерах разгонялся до 7-8s, давая flaky-падения. 10s оставляет
        # запас x2 над типичным временем под нагрузкой и всё ещё ловит реальный
        # O(n²) → O(n³) регресс (где время выросло бы до десятков секунд).
        assert opt_time < 10.0, f"TSP оптимизация слотов заняла {opt_time:.2f}s, ожидалось < 10s"
