...
def redraw_grid(event=None):
    ...
    # Отрисовка контура платы (Gerber outline)
    if cfg.board_outline and cfg.board_outline_visible:
        _draw_board_outline(canvas, wa_x1, wa_y1, wa_x2, wa_y2)

    # Отрисовка нарушений (отверстия за пределами контура)
    if cfg.outline_violations:
        for desc, (x_mm, y_mm) in cfg.outline_violations:
            real_x = to_real_x(x_mm)
            real_y = to_real_y(y_mm)
            if is_in_workarea(real_x, real_y):
